...
class FeedSnapshotTests: XCTestCase {
    ...
    func test_feedWithLoadMoreError() {
        ...
        assert(snapshot: sut.snapshot(for: .iPhone13(style: .dark,contentSize: .extraExtraExtraLarge)), named: "FEED_WITH_LOAD_MORE_ERROR_dark_extraExtraExtraLarge")
    }
    ...
    private func feedWithLoadMoreError() -> [CellController] {
        let loadMore = LoadMoreCellController()
        loadMore.display(ResourceErrorViewModel(message: "This is a mulitline\nerror message"))
        return feedWith(loadMore: loadMore)
    }
    
    private func feedWith(loadMore: LoadMoreCellController) -> [CellController] {
        let stub = feedWithContent().last!
        let cellController: FeedImageCellController = FeedImageCellController(viewModel: stub.viewModel,delegate: stub,selection: {})
